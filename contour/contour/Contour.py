    ...
    for j in range(nosas_lon):
        ...
        if(k==25 or (i==(nosas_lat-1) and j==(nosas_lon-1))):
            ...
            for kk in range(length):
                ...
                res.append(results[kk]['resolution'])
	    ...
	    for kk in range(length):
                ...
                for i in range(len(tstr)):
                    if(tstr[i]!=' 'and tstr[i]!='\t'and tstr[i]!='\n'):
                        lflg=1
                if(lflg==1):
                    f.write(str(lng[kk])+'\t'+str(lat[kk])+'\t'+str(e[kk])+'\t'+str(res[kk])+'\n')
                lflg=0
...
plt.savefig(pathtrue+".png",dpi=1000,transparent=False,bbox_inches='tight')
